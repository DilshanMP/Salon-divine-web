...
if(isset($_SESSION['user_details'])){
	...
	foreach($_SESSION['user_details'] as $key => $value){
		...
		$sql = "SELECT * FROM booking where user_id = '".$value['email']."'";
	}
	...
	while($row= mysqli_fetch_row($result)){
		...
		if(isset($_SESSION['booking_details'])){
			...
			'user_id'=>$row[5]); 
			$count = count($_SESSION['booking_details']);
		
		 	$_SESSION['booking_details'][$count]=$BookingsArray;
			...
			'user_id'=>$row[5]); 
		 	...
		 	$_SESSION['booking_details'][0]=$BookingsArray;
		}
...
//if add a service already in booking list

if(isset($_SESSION['bookingErro'])){
	
	?>
    
    	<script> swal("<?php echo $_SESSION['bookingErro']; ?>", "Press Ok", "warning");</script>
    
	<?php
	
	unset($_SESSION['bookingErro']);
}
...
<div class="container pb-5 mb-5" style="border-bottom:solid 1px #CCC">
    	...
    	<div class="col-lg-12 mt-5">
            ...
            <table class="table table-light" >
            	<tr>
                    ...
                    <td align="right" <?php if(isset($_POST['btnViewBooking'])){ ?> style="display:none"<?php }else{?> style="display:block" 
					<?php } ?>>
                    	<button style="border:none; background:#FFF; outline:none;" data-toggle="modal" data-target="#myBookingList"><svg 
						width="3em" height="3em" viewBox="0 0 16 16" class="bi bi-journal-text" fill="currentColor" xmlns=
						"http://www.w3.org/2000/svg"><path d="M3 0h10a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2v-1h1v1a1 1 0 0 0 1 1h10a1
						1 0 0 0 1-1V2a1 1 0 0 0-1-1H3a1 1 0 0 0-1 1v1H1V2a2 2 0 0 1 2-2z"/><path d="M1 5v-.5a.5.5 0 0 1 1 0V5h.5a.5.5 0 0 1 0 
						1h-2a.5.5 0 0 1 0-1H1zm0 3v-.5a.5.5 0 0 1 1 0V8h.5a.5.5 0 0 1 0 1h-2a.5.5 0 0 1 0-1H1zm0 3v-.5a.5.5 0 0 1 1 
						0v.5h.5a.5.5 0 0 1 0 1h-2a.5.5 0 0 1 0-1H1z"/><path fill-rule="evenodd" d="M5 10.5a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 
						1h-2a.5.5 0 0 1-.5-.5zm0-2a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5zm0-2a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 
						1h-5a.5.5 0 0 1-.5-.5zm0-2a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5z"/></svg>
						<?php if (isset($_SESSION['booking_items'])) {?><span style="background:#C03; padding:3px 10px 5px 10px; border-radius
                        :50%; color:#FFF;"><?php echo $_SESSION['booking_items'];?></span><?php } ?></button>
                    </td>
    ...
    <div class="row" <?php if(isset($_POST['btnViewBooking'])){ ?> style="display:none"<?php }else{?> style="display:block" <?php } ?> >
        ...
        <div class="col-lg-12 mt-0">
        	<table class="table table-active" style="background:#f5f5f5; text-align:left">
                ...
                <form action="<?php echo $_SERVER['PHP_SELF'];?>?id=<?php echo $row[0];?>" method="post">
                	<td>
						<?php echo $row[1];?>
                    	<input type="hidden" name="service_name" value="<?php echo $row[1]; ?>"/>
                    </td>
                    <td style="text-align:right">
                    	Rs.<?php echo $row[3];?>.0
                        <input type="hidden" name="service_price" value="<?php echo $row[3]; ?>"/>
                    </td>
    ...
    <table class="table table-active" style="background:#f5f5f5;-webkit-box-shadow: 0px 5px 8px #333333;">
    	...
    	<tr style="color:#333; font-size:14px; font-weight:500">
        	...
        	<td><button type="submit" class="btn btn-info" data-toggle="modal" data-target="#<?php echo $value['b_id'];?>">More</button>
            </td>
    	</tr>
